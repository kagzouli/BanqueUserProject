 ...
 */
Ext.define('Ext.ux.form.PblinkItemSelector', {
    extend: 'Ext.ux.form.MultiSelect',
    alias: ['widget.pblinkitemselectorfield', 'widget.pblinkitemselector'],
    alternateClassName: ['Ext.ux.PblinkItemSelector'],
    requires: [
        'Ext.button.Button',
     ...
     */
    buttonsText: {
        top: "$common.pblinkgridreconfigwindow-moveTop",
        up: "$common.pblinkgridreconfigwindow-moveUp",
        add: "$common.pblinkgridreconfigwindow-add",
        remove: "$common.pblinkgridreconfigwindow-remove",
        down: "$common.pblinkgridreconfigwindow-moveDown",
        bottom: "$common.pblinkgridreconfigwindow-moveBottom"
    },
    ...
    onRemoveBtnClick : function() {
        ...
        Ext.Array.each(me.fromField.boundList.getSelectionModel().getSelection(), function(record, index) {
        	record.reject();
        });
    },
	...
	updateHTMLElements : function() {
		var me = this;
		var boundList = this.toField.boundList;
		var records = boundList.getStore().getRange();

		Ext.Array.each(records, function(record, index, array) {
			if (record.get("sortState")) {
				me.changeHTMLAttribute("add",
						boundList.getNodeByRecord(record), "sortstate", record
								.get("sortState").toLowerCase());
			}
			if (record.get("grouping")) {
				me.changeHTMLAttribute("add",
						boundList.getNodeByRecord(record), "grouping", record
								.get("grouping"));
			}			
		});
	},
	changeHTMLAttribute : function(type, el, attributeName, attributeValue) {
		switch (type) {
		case ("add"):
